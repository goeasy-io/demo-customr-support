...
confirm_version() {
    ...
    if [ "$action" = "r" ]; then
        ...
        cd support/web
        currentVersion=$(npm run env | grep npm_package_version | cut -d '=' -f 2)
        versionDir="show-cs/"$currentVersion
    fi
    # 退回根目录
    cd ../../
...
build_web() {
    ...
    mv dist ../../build/$versionDir/agent
    cd ../../
...
# 构建custiner服务
build_customer() {
    echo "----------start execute build_customer----------"
    cd customer/uniapp
    npm ci
    npm run build -- --appkey=$config_appkey
    mv dist/build/h5 ../../build/$versionDir/customer
    rm -rf dist
    cd ../../
    echo "----------end execute build_customer----------"
}
...
copy_html() {
    ...
    # 替换index.html中的路径
    basePath="\/show-cs\/$versionDir"
    customerPath=src\=$basePath\\/customer\\/
    agentPath=src\=$basePath\\/agent\\/
    sed -i "s/src\=\"customer\/\"/$customerPath/g" build/$versionDir/index.html
    sed -i "s/src\=\"agent\/\"/$agentPath/g" build/$versionDir/index.html

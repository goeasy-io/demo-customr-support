	...
	*/
	...
	function () {
	  ...
	  Conversation.prototype.toDto = function (history) {
	    var conversationDto = new GoEasy_1$c.ConversationDTO();
	    conversationDto.type = this.scene;

	    if (this.scene === GoEasy_1$c.Scene.PRIVATE) {
	      conversationDto.userId = this.targetId;
	    } else if (this.scene === GoEasy_1$c.Scene.GROUP) {
	      conversationDto.groupId = this.targetId;
	    } else if (this.scene === GoEasy_1$c.Scene.CS) {
	...
	function () {
	  ...
	  Conversations.prototype.findConversationIndex = function (scene, targetId) {
	    return this.list.findIndex(function (item) {
	      return scene === GoEasy_1$7.Scene.GROUP && targetId === item.targetId || scene === GoEasy_1$7.Scene.PRIVATE && targetId === item.targetId || scene === GoEasy_1$7.Scene.CS && targetId === item.targetId;
	    });
	  };
	...
	function () {
	  ...
	  Target.byIds = function (userId, groupId) {
	    var scene = GoEasy_1$6.Scene.PRIVATE;
	    var targetId = userId;

	    if (Calibrator_1$8["default"].isDef(groupId)) {
	      scene = GoEasy_1$6.Scene.GROUP;
	      targetId = groupId;
	    }
	...
	function () {
	  ...
	  IMMessageBuilder.prototype.build = function (buildOptions) {
	    ...
	    message.status = GoEasy_1$5.MessageStatus.NEW;
	  ...
	  IMMessageBuilder.prototype.validate = function (createOptions) {
	    ...
	    if (!to.type || to.type !== GoEasy_1$5.Scene.GROUP && to.type !== GoEasy_1$5.Scene.PRIVATE && to.type !== GoEasy_1$5.Scene.CS) {
	...
	function () {
	  function DeleteMessageRequest(messages) {
	    ...
	    messages.forEach(function (message) {
	      if (message.status === GoEasy_1$4.MessageStatus.SUCCESS) {
	        //未成功的消息，不要发到服务端
	...
	(function (exports) {
	  ...
	  function () {
	    ...
	    Histories.prototype.markCSMessageAsRead = function (markOption) {
	      var target = Target_1.Target.byScene(markOption.scene, markOption.id);
	      this.markAsRead(target, markOption);
	    };
	    ...
	    Histories.prototype.markAsRead = function (target, markOption) {
	      return __awaiter(this, void 0, void 0, function () {
	        ...
	        return __generator(this, function (_a) {
	          switch (_a.label) {
	            case 0:
	              if (target.scene === GoEasy_1.Scene.PRIVATE) {
	                key = 'userId';
	              } else if (target.scene === GoEasy_1.Scene.GROUP) {
	                key = 'groupId';
	              } else if (target.scene === GoEasy_1.Scene.CS) {
	                key = 'id';
	              }

	              validator_utils_1["default"].validateId(target.id, key);
	              markOption[key] = markOption[key].toString();
	...
	function () {
	  ...
	  Team.prototype.setTeamId = function (imMessage) {
	    if (imMessage.scene() === GoEasy_1$3.Scene.CS) {
	...
	function () {
	  ...
	  IM.prototype.markMessageAsRead = function (options, teamId) {
	    var _this = this;

	    this["catch"](function () {
	      if (GoEasy_1$2.Scene.CS == options.scene) {
	        if (teamId) {
	          //cs staff calls this function
	          options.id = options.id + "#" + teamId;
	        } else {
	          //customer calls this function
	          options.id = IM.userId + "#" + options.id;
	        }
	      }

	      return _this.histories.markCSMessageAsRead(options);
	    }, options);
	  };
